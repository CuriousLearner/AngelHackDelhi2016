...
function NewValueImpactScore() {
    ...
    for (i = 0; i < data.length; i++) {
        if (max_impact_score < data[i].impact_score) {
            max_impact_score = data[i].impact_score;
            config0.maxValue = max_impact_score * 1.1;
        }
        if (min_impact_score > data[i].impact_score) {
            min_impact_score = data[i].impact_score;
            config0.minValue = min_impact_score;
        }
        total_impact_score = total_impact_score + data[i].impact_score;
    }
    var avg_impact_score = total_impact_score / data.length;
...
function NewValueRetweet() {
    ...
    var data = getData()
    for (i = 0; i < data.length; i++) {
        if (max_retweet < data[i].total_retweet_count) {
            max_retweet = data[i].total_retweet_count;
            config1.maxValue = max_retweet * 1.1;
        }
        if (min_retweet > data[i].total_retweet_count) {
            min_retweet = data[i].total_retweet_count;
            config1.minValue = min_retweet;
        }
        total_retweet = total_retweet + data[i].total_retweet_count;
    }
    var avg_retweet = total_retweet / data.length;
...
function NewValueFavorite() {
    ...
    var data = getData()
    for (i = 0; i < data.length; i++) {
        if (max_favorites < data[i].total_fav_count) {
            max_favorites = data[i].total_fav_count;
            config2.maxValue = max_favorites * 1.1;
        }
        if (min_favorites > data[i].total_fav_count) {
            min_favorites = data[i].total_fav_count;
            config2.minValue = min_favorites;
        }
        total_favorites = total_favorites + data[i].total_fav_count;
    }
    var avg_favorites = total_favorites / data.length;
...
function NewValueFollower() {
    ...
    var data = getData()
    for (i = 0; i < data.length; i++) {
        if (max_follower < data[i].follower_count) {
            max_follower = data[i].follower_count;
            config3.maxValue = max_follower * 1.1;
        }
        if (min_follower > data[i].follower_count) {
            min_follower = data[i].follower_count;
            config3.minValue = min_follower;
        }
        total_follower = total_follower + data[i].follower_count;
    }
    var avg_follower = total_follower / data.length;
    ...
    function handleSearch(message, handlename) {
        ...
        $.each(message, function(i, e) {
            if (message[i].twitter_username == handlename) {
                ...
                adddata = adddata.replace('impactScore">', 'impactScore">' + Math.round(message[i].impact_score * 100));
